...
# Plot log(gene_expression) ~ concentration with appropriate styling for data1
plotA <- ggplot(data1,aes(x=conc,y=log(gene_expression)))+
  ggtitle("Wild-type")+
...
# Plot log(gene_expression) ~ concentration with appropriate styling for data2
plotB <- ggplot(data2,aes(x=conc,y=log(gene_expression)))+
...
# Save figure
ggsave("figs/5-4-24-LogKarlBeratorPlot.tiff", finalPlot, width = 9, height = 6, dpi = 500, device = "tiff")
